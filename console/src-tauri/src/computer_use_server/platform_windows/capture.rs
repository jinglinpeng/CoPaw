...
use super::super::state::{
    ...
    ServerState, WindowInfo, BMP_HEADER_BYTES, SCREENSHOT_JPEG_QUALITY, SCREENSHOT_MAX_EDGE,
};
...
pub(crate) fn observe_window(
    ...
    let accessibility = options
        .include_text
        .then(|| collect_accessibility(windows.input_hwnd));
    require_observation_source(options, &captures, accessibility.as_ref())?;
    ...
    let window_bounds = get_visible_window_rect(HWND(window.hwnd as _))
        .map(rect_bounds)
        .unwrap_or_default();
    let capture = capture_parts(options.include_screenshot, captures);
...
fn require_observation_source<A>(
    ...
    accessibility: Option<&Result<A, String>>,
    ...
    if captures.iter().any(|capture| capture.result.is_ok())
        || accessibility.is_some_and(Result::is_ok)
    {
        return Ok(());
    }
...
fn capture_parts(requested: bool, captures: Vec<WindowCapture>) -> CaptureParts {
...
mod tests {
    ...
    use crate::computer_use_server::state::ObservationOptions;
    ...
    fn observation_fails_when_both_sources_fail() {
        ...
        let error = require_observation_source(
            ObservationOptions::default(),
            &captures,
            Some(&accessibility),
        )
